...
%%%%%%%%%%%%
% Input:
% Sig: A sample from the posterior of the corresponding covariance in the
%      expanded space.
% w : The latent Gaussian data in the expanded space (w=zD)
% X : The n x p design matrix 
% Xi: updated posterior parameter Xi = inv(X'X+psi)
%
% Output
% betahat: A sample from the posterior of beta
% M: Xi*X'W

J=length(Sig);
    [n p]=size(X);
    L=kron(Sig,Xi); % [[sig(1,1)*Xi], .... , [sig(J,J)*Xi]] (J*p) x (J*p)  .
    M=Xi*X'*w;
    B=reshape(M,p*J,1); %  make row from matrix  (p*J) x 1.
    betahat=reshape(mvg(B,L,1),p,J);%mu is mx1 and Sigma is mxm and SPD, produces  an mxN matrix y
%%%%%%%
